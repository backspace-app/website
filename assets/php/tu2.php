...
if($_POST){
	...
	if ($_POST['session'] == $info['session']) { //looks to see if the session is current
		...
		$stmt = $db->prepare("SELECT user_id FROM spwp_appprofile WHERE spwp_appprofile.username=?");
		...
		$stmt->execute(array($search[0]['user_id'],$search[0]['user_id'],$search[0]['user_id']));
		...
		if ($info['id'] == $search[0]['user_id']) { //determines if the profile is the current user's
			...
			$stmt->execute(array(':one' => $info['id'],':two' => $search[0]['user_id']));
		...
		if ($friend > 0 || $user['is_private'] == 0) { //determines what information to show based on user's security settings
			if ($info['id'] == $search[0]['user_id']) { //show all posts if the current profile is the user's profile
				...
				$stmt->execute(array($search[0]['user_id'],time(),$page_start,$page_end));
			}
			else {
				...
				if ($num_words > 0) {
					...
					if ($friend > 0){ //show all posts the user is allowed to see...following, friend, friends group
						...
						$stmt->execute(array($search[0]['user_id'],time(),$search[0]['user_id'],$info['id'],time(),$page_start,$page_end));
					}
					...
					if ($friend > 0){ //show all posts the user is allowed to see...following, friend, friends group
						...
						$stmt->execute(array($search[0]['user_id'],time(),$search[0]['user_id'],$search[0]['user_id'],time(),$page_start,$page_end));
					}
					else { //only show public posts
