...
if($_POST){
	...
	if ($_POST['session'] == $info['session']) { //verifies the current session ID is active
		//$stmt = $db->prepare("SELECT profile_pic, COUNT(msg_id) AS msg_id, COUNT(notif_id) AS notif_id FROM spwp_appprofile LEFT JOIN spwp_appmessages ON spwp_appmessages.receiver =  spwp_appprofile.user_id AND spwp_appmessages.viewed <> 1 LEFT JOIN spwp_appnotifications ON spwp_appnotifications.poster = spwp_appprofile.user_id AND spwp_appnotifications.viewed <> 1 WHERE spwp_appprofile.user_id=:id");
		$stmt = $db->prepare("SELECT profile_pic, COUNT(notif_id) AS notif_id FROM spwp_appprofile LEFT JOIN spwp_appnotifications ON spwp_appnotifications.poster = spwp_appprofile.user_id AND spwp_appnotifications.viewed <> 1 WHERE spwp_appprofile.user_id=:id");
		$stmt->execute(array(':id' => $info['id']));
		$userinfo = $stmt->fetch(PDO::FETCH_ASSOC); //gets information on current user
		//paging
		if ($_POST['page'] == '') {
			$page_start = 0;
		}
		...
		if ($num_words > 0) {
			...
			$stmt = $db->prepare("SELECT post_id, type, username, poster, profile_pic, content, pic, thumb, length FROM (SELECT post_id, type, username, poster, profile_pic, content, pic, thumb, length FROM spwp_appposts JOIN spwp_appprofile ON spwp_appposts.poster = spwp_appprofile.user_id WHERE spwp_appprofile.user_id=? AND spwp_appposts.length>? UNION ALL SELECT post_id, type, username, poster, profile_pic, content, pic, thumb, length FROM spwp_appposts JOIN spwp_appprofile ON spwp_appposts.poster = spwp_appprofile.user_id JOIN spwp_appfollowing ON spwp_appposts.poster = spwp_appfollowing.user_two WHERE spwp_appfollowing.user_one=? AND spwp_appposts.group_id='0' AND spwp_appposts.length>? AND LOWER(spwp_appposts.content) NOT REGEXP ('".$words."') UNION ALL SELECT post_id, type, username, poster, profile_pic, content, pic, thumb, length FROM spwp_appposts JOIN spwp_appprofile ON spwp_appposts.poster = spwp_appprofile.user_id JOIN spwp_appfriends ON spwp_appposts.poster = spwp_appfriends.user_two WHERE spwp_appfriends.user_one=? AND spwp_appfriends.pending <>1 AND spwp_appposts.group_id IN ('0','1') AND spwp_appposts.length>? AND LOWER(spwp_appposts.content) NOT REGEXP ('".$words."') UNION ALL SELECT post_id, type, username, poster, profile_pic, content, pic, thumb, length FROM spwp_appposts JOIN spwp_appprofile ON spwp_appposts.poster = spwp_appprofile.user_id JOIN spwp_appgroups ON spwp_appposts.group_id = spwp_appgroups.group_id WHERE spwp_appgroups.group_member=? AND spwp_appposts.length>? AND LOWER(spwp_appposts.content) NOT REGEXP ('".$words."'))posts ORDER BY posts.length DESC LIMIT ?,?"); //gets post information
		}
		else {
			$stmt = $db->prepare("SELECT post_id, type, username, poster, profile_pic, content, pic, thumb, length FROM (SELECT post_id, type, username, poster, profile_pic, content, pic, thumb, length FROM spwp_appposts JOIN spwp_appprofile ON spwp_appposts.poster = spwp_appprofile.user_id WHERE spwp_appprofile.user_id=? AND spwp_appposts.length>? UNION ALL SELECT post_id, type, username, poster, profile_pic, content, pic, thumb, length FROM spwp_appposts JOIN spwp_appprofile ON spwp_appposts.poster = spwp_appprofile.user_id JOIN spwp_appfollowing ON spwp_appposts.poster = spwp_appfollowing.user_two WHERE spwp_appfollowing.user_one=? AND spwp_appposts.group_id='0' AND spwp_appposts.length>? UNION ALL SELECT post_id, type, username, poster, profile_pic, content, pic, thumb, length FROM spwp_appposts JOIN spwp_appprofile ON spwp_appposts.poster = spwp_appprofile.user_id JOIN spwp_appfriends ON spwp_appposts.poster = spwp_appfriends.user_two WHERE spwp_appfriends.user_one=? AND spwp_appfriends.pending <>1 AND spwp_appposts.group_id IN ('0','1') AND spwp_appposts.length>? UNION ALL SELECT post_id, type, username, poster, profile_pic, content, pic, thumb, length FROM spwp_appposts JOIN spwp_appprofile ON spwp_appposts.poster = spwp_appprofile.user_id JOIN spwp_appgroups ON spwp_appposts.group_id = spwp_appgroups.group_id WHERE spwp_appgroups.group_member=? AND spwp_appposts.length>?)posts ORDER BY posts.length DESC LIMIT ?,?"); //gets post information
		}
		//end blocked words
		
		$stmt->execute(array($info['id'],time(),$info['id'],time(),$info['id'],time(),$info['id'],time(),$page_start,$page_end));
		...
		$read_out = '{"success":1,"notifications":'.$userinfo['notif_id'].',"items":[';
