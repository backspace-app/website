...
if($_POST){
	...
	if (($_POST['session'] == $info['session']) && $_POST['search'] <> "") { //looks to see if the session is current and if there is a search term
		$stmt = $db->prepare("SELECT user_id, username, profile_pic FROM spwp_appprofile JOIN spwp_appfriends ON spwp_appprofile.user_id = spwp_appfriends.user_two WHERE spwp_appprofile.username LIKE :search_term AND spwp_appfriends.user_one=:one and spwp_appfriends.pending=0");
		$stmt->execute(array(':search_term' => '%'.$_POST['search'].'%', ':one' => $info['id']));
